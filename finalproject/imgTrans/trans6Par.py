...
def get6Par(RGB1, RGB2):
    """Compute transformation parameters"""
    r1, g1, b1 = np.hsplit(RGB1, 3)
    r2, g2, b2 = np.hsplit(RGB2, 3)

    # Form coefficient matrix B and constants matrix f
    Br = np.array(map(lambda r: [r[0], 0, 0, 1, 0, 0], r2))
    Bg = np.array(map(lambda g: [0, g[0], 0, 0, 1, 0], g2))
    Bb = np.array(map(lambda b: [0, 0, b[0], 0, 0, 1], b2))
    ...
    print "%-10s %8.4f %9.4f" % ("s1", X[0, 0], param_std[0])
    print "%-10s %8.4f %9.4f" % ("s2", X[1, 0], param_std[1])
    print "%-10s %8.4f %9.4f" % ("s3", X[2, 0], param_std[2])
    print "%-10s %8.4f %9.4f" % ("t1", X[3, 0], param_std[3])
    print "%-10s %8.4f %9.4f" % ("t2", X[4, 0], param_std[4])
    print "%-10s %8.4f %9.4f" % ("t3", X[5, 0], param_std[5])
...
def transImg6par(Img, param, outputImgName):
    """Use given six parameters to transform the image color"""
    s1, s2, s3, t1, t2, t3 = param
...
def main():
    ...
    # Transform image color
    transImg6par(queryImgRGB, X, outputImgName)
